    ...
    <section className="bg-dalia-navy text-white py-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center max-w-3xl mx-auto mb-14 space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/10 text-dalia-gold text-xs font-bold border border-dalia-gold/30">
            <Sparkles size={14} />
          ...
          <p className="text-base md:text-lg text-dalia-slate-light leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {techCards.map((card) => (
            <div
              key={card.id}
              className="bg-white/5 backdrop-blur-md rounded-3xl overflow-hidden border border-white/10 hover:border-dalia-gold/60 transition-all duration-300 group flex flex-col justify-between"
            >
              <div>
                <div className="relative h-48 w-full overflow-hidden bg-dalia-navy-dark">
                  <Image
                    src={card.image}
                    alt={card.title}
                    fill
                  ...
                  <div className="absolute top-4 left-4 bg-dalia-navy/90 backdrop-blur-md text-dalia-gold text-xs font-bold px-3 py-1 rounded-full border border-dalia-gold/30 flex items-center gap-1.5">
                    {card.icon}
                    <span>{card.tag}</span>
                  </div>
                  <div className="absolute bottom-3 right-4 bg-dalia-gold text-dalia-navy text-[11px] font-bold px-2.5 py-0.5 rounded-md">
                    {card.highlight}
                  </div>
                </div>

                <div className="p-6 space-y-3">
                  <h3 className="font-serif text-xl font-bold text-white group-hover:text-dalia-gold transition-colors">
                    {card.title}
                  </h3>
                  <p className="text-sm text-dalia-slate-light leading-relaxed">
                    {card.description}
                  </p>
                </div>
              </div>
              ...
              <div className="p-6 pt-0 border-t border-white/10 mt-4 flex items-center justify-between">
                <Link
                  href={card.link}
                  className="text-xs font-semibold text-dalia-gold hover:text-white flex items-center gap-1 transition-colors"
                >
                ...
                <button
                  ...
                  className="btn-gold text-xs px-3.5 py-1.5 rounded-lg font-semibold flex items-center gap-1"
                >
                  <Calendar size={13} />
                  <span>Probar</span>
                </button>
        ...
        <div className="mt-12 text-center">
          <button
            onClick={() => openAppointment()}
            className="btn-gold px-8 py-3.5 rounded-2xl text-sm font-semibold inline-flex items-center gap-2"
          >
            <Calendar size={16} />
