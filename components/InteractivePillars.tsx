    ...
    <div className="w-full">
        ...
        {pillars.map((p, idx) => {
          ...
          return (
            <button
              ...
              className={`p-4 rounded-2xl text-left border transition-all duration-300 flex items-center gap-3 ${
                isActive
                  ? "bg-dalia-navy text-white border-dalia-gold shadow-luxury scale-[1.02]"
                  : "bg-white text-dalia-graphite border-dalia-warm hover:border-dalia-gold/50 hover:bg-dalia-warm-light"
              }`}
            >
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
                  isActive
                    ...
                    : "bg-dalia-rose/30 text-dalia-navy"
                }`}
              >
                {p.icon}
              </div>
              <div className="min-w-0">
                <p className="font-serif text-sm font-bold truncate leading-tight">
                  {p.title}
                </p>
                <p
                  className={`text-[11px] truncate mt-0.5 ${
                    isActive ? "text-dalia-slate-light" : "text-dalia-graphite-light"
                  }`}
                >
                  {p.tag}
                </p>
      ...
      <div className="bg-white rounded-3xl p-6 md:p-10 border border-dalia-gold/30 shadow-luxury grid grid-cols-1 lg:grid-cols-12 gap-8 items-center animate-in fade-in duration-300">
        ...
        <div className="lg:col-span-6 space-y-5">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-dalia-rose/30 text-dalia-navy text-xs font-bold border border-dalia-rose">
            <Sparkles size={14} className="text-dalia-gold" />
            <span>{current.tag}</span>
          </div>

          <div>
            <h3 className="font-serif text-3xl md:text-4xl font-bold text-dalia-navy">
              {current.title}
            </h3>
            <p className="text-sm font-semibold text-dalia-gold-dark mt-1">
              {current.subtitle}
            </p>
          </div>
          ...
          <div className="space-y-2.5 pt-2 border-t border-dalia-warm/60">
            {current.points.map((pt, i) => (
              <div key={i} className="flex items-center gap-2.5 text-sm font-medium text-dalia-navy">
                <CheckCircle2 size={17} className="text-dalia-gold shrink-0" />
                <span>{pt}</span>
              </div>
            ))}
          </div>
          ...
          <div className="pt-2">
            <Link
              href="/quienes-somos"
              className="inline-flex items-center gap-2 text-sm font-bold text-dalia-navy hover:text-dalia-gold transition-colors"
            >
        ...
        <div className="lg:col-span-6">
          <div className="relative h-72 sm:h-96 w-full rounded-2xl overflow-hidden border border-dalia-gold/30 shadow-md">
            <Image
              src={current.image}
              alt={current.title}
              fill
            ...
            <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md p-3.5 rounded-xl border border-dalia-gold/30">
              <p className="text-xs font-bold text-dalia-navy uppercase tracking-wider">
                Compromiso Dalia Clinic
              </p>
              <p className="text-xs text-dalia-graphite mt-0.5">
                Cada procedimiento es planificado con dedicación exclusiva y atención personalizada.
              </p>
            </div>
          </div>
        </div>
